...
class Layout : public sf::Drawable, sf::Transformable {
 public:
  ...
  RoadPhysical* createRoad(IntersectionPhysical& start,
                           IntersectionPhysical& end, std::string datapath);

  // physical ID
  void removeRoad(size_t id);

  // returns nullptr if no intersections
  std::pair<IntersectionPhysical*, double> findClosestIntersection(
      sf::Vector2f pos);
  std::pair<RoadPhysical*, double> findClosestRoad(sf::Vector2f pos);

  Road* getRoad(size_t id) const { return _roads.at(id); }
  RoadPhysical* getPhysicalRoad(size_t id) const {
    return _physicalRoads.at(id).get();
  }
  ...
  Intersection* getIntersectionFromInternalID(size_t id) const {
    return _intersections.at(id);
  }
  IntersectionPhysical* getIntersectionPhysicalFromInternalID(size_t id) const;
  ...
  Road* getRoadBetweenTwoIntersectionsFromInternalID(size_t begin,
                                                     size_t end) const;
