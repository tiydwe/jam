...
bool Car::_mergeToNextLane(double dd) {
  ...
  double dperc =
      _position_distance /
      (_parentSim->getLane(_position_laneid)->getLane()->getLength() - _margin);
  double chance = utility::cumlitivePtoSlice(
      utility::smoothstep, dperc,
      dd / (_parentSim->getLane(_position_laneid)->getLane()->getLength() -
            _margin));
