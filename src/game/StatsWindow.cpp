      ...
      _stats(stats)
{
  ...
  _continue.setOnclick([&](Game *game)
                       { game->endStatsScreen(); });
}
...
std::string StatsWindow::getStatsString()
{
  ...
  res << "Stats:\nCars arrived: " << std::fixed << std::setprecision(2)
      << (_stats.numberOfCars == 0 ? 0 : ((double)_stats.numberArrived) / _stats.numberOfCars * 100)
      << "%\n"
      << "Average speed: "
      << (utility::isclose(_stats.totalTimeTraveled, 0.0) ? 0 : _stats.totalDistanceTraveled / _stats.totalTimeTraveled)
      << " speed units\n";
  return res.str();
}
